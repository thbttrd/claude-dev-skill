...
3. **FAIL**: list critical issues. Outside autopilot ask whether to fix now (loops back into `/plan-writing US-NNN` with `--force`). Under autopilot: hard stop `verifier_fail` (contract §2).
