...
// Project-wide artifact path: drop both the legacy version segment AND the
// architecture/specs/plans subdirectory, since the canonical layout has these
// files at `specs/` directly (e.g. `specs/ARCHITECTURE.md`, `specs/architecture.png`).
function toProjectArtifactPath(p) {
  if (!p || typeof p !== "string") return p;
  return p
    .replace(/^docs\/V\d+\/(?:architecture|specs|plans)\//, "specs/")
    .replace(/^docs\/V\d+\//, "specs/")
    .replace(/^docs\//, "specs/");
}

// Per-story artifact path: route to the canonical
// `specs/story-<NNN>-<slug>/<subdir>/<filename>` location regardless of where
// the legacy file lived. The migrate-specs SKILL is responsible for the actual
// file move; this function just emits the destination path so stories.json
// already points at the right place once moves complete.
function toStoryArtifactPath(p, storyId, slug, subdir) {
  if (!p || typeof p !== "string") return p;
  const filename = p.split("/").pop();
  const numericId = String(storyId).replace(/^US-?/, "").padStart(3, "0");
  return `specs/story-${numericId}-${slug}/${subdir}/${filename}`;
}
...
function migrate(legacy) {
    ...
    for (const us of e.user_stories ?? []) {
      ...
      const story = {
        ...
        artifacts: {
          story_doc: null,
          plan: null,
          feature_files: us.spec?.feature_file
            ? [toStoryArtifactPath(us.spec.feature_file, us.id, slugify(us.title || us.id), "features")]
            : [],
          mockups: us.ui?.mockup_desktop
            ? [toStoryArtifactPath(us.ui.mockup_desktop, us.id, slugify(us.title || us.id), "mockups")]
            : [],
          ui_specs: us.ui?.screen_spec
            ? [toStoryArtifactPath(us.ui.screen_spec, us.id, slugify(us.title || us.id), "ui")]
            : [],
          qa_report: null,
        },
  ...
  const architecture = {
    ...
    diagram_path: toProjectArtifactPath(arch.diagram_path) ?? "specs/architecture.png",
  };
  ...
  if (arch.detailed_diagram_path)
    architecture.detailed_diagram_path = toProjectArtifactPath(arch.detailed_diagram_path);
  if (arch.architecture_doc)
    architecture.architecture_doc = toProjectArtifactPath(arch.architecture_doc);
...
function renderMigrationMd(data, opts) {
  ...
  for (const s of data.stories) {
    ...
    for (const f of s.artifacts.feature_files ?? [])
      moves.push(`mv docs/V<latest>/specs/features/${basename(f)} ${dir}/features/`);
    for (const m of s.artifacts.mockups ?? [])
      moves.push(`mv docs/V<latest>/specs/mockups/${basename(m)} ${dir}/mockups/`);
    for (const u of s.artifacts.ui_specs ?? [])
      moves.push(`mv docs/V<latest>/specs/${basename(u)} ${dir}/ui/`);
  }
